...
class model(object):
    ...
    def _cnn(self, X_embedded):
        ...
        for filter_size in filter_size_list:
            ...
            h_total.append(h)
        if len(h_total)>1:
            max_filter_size=max(filter_size_list)
            min_len=general_config.max_seq_len-max_filter_size+1
            h=tf.concat([h[:,:min_len,:] for h in h_total],axis=2)
            h=tf.reshape(h,shape=[tf.shape(self.X)[0],min_len,self.filter_num*len(h_total)])
        else:
            h=h_total[0]
        return h
    ...
    def _rnn(self, rnn_inputs):
        ...
        rnn_outputs_fw = rnn_outputs_fw[:,-1,:]
        rnn_outputs_bw = rnn_outputs_bw[:,0,:]
    ...
    def _feed_dict_train(self, batch_x, batch_y):
        feed_dict = {self.X: batch_x, self.y: batch_y,
                     self.dropout: self.dropout_value, self.learning_rate: self.learning_rate_value}
        return feed_dict

    def _feed_dict_valid(self, batch_x, batch_y):
        feed_dict = {self.X: batch_x, self.y: batch_y}
        return feed_dict

    def _feed_dict_test(self, batch_x):
        ...
        with tf.Session(config=config,graph=self.graph) as sess:
            ...
            while (test_generator.loop == cur_loop):
                predicted = sess.run(self.predicted,
                                     feed_dict=self._feed_dict_test(batch_seqs))
